...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    ...
    // Combine both loops into one to iterate over apples and oranges
    for (size_t i = 0; i < apples.size() || i < oranges.size(); i++) {
        // Process apples if within bounds
        if (i < apples.size() && (a + apples[i] >= s && a + apples[i] <= t)) {
            apple_cnt++;
        }
        // Process oranges if within bounds
        if (i < oranges.size() && (b + oranges[i] >= s && b + oranges[i] <= t)) {
            orange_cnt++;
        }
    }

    cout << apple_cnt << endl;
    cout << orange_cnt << endl;
}
